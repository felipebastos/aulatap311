...
@click.group()
def manage():
    '''This script manages the aulatap311 project.'''
...
def runserver(port):
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'. .venv/bin/activate; export FLASK_APP=aulatap311.app; export FLASK_ENV=development; flask run --port={port}'
    elif os.name == 'nt':
        cmd = f'call .venv/Scripts/activate; set FLASK_APP=aulatap311.app; set FLASK_ENV=development; flask run --port={port}'
...
def plug_blueprint(name: str, templates: bool):
    ...
    os.mkdir(os.path.join(os.getcwd(), 'aulatap311', 'blueprints', name))
    if templates:
        tf = os.path.join(
            os.getcwd(), 'aulatap311', 'blueprints', name, 'templates', name)
        os.makedirs(tf)
        click.echo(f"Placed this blueprint's templates under {tf}")

    init = open(os.path.join(
        os.getcwd(), 'aulatap311', 'blueprints', name, '__init__.py'), 'w')
    init.close()
    with open(os.path.join(os.getcwd(), 'aulatap311', 'blueprints', name, f'{name}.py'), 'w') as blueprint:
    ...
    settings['default']['EXTENSIONS'].append(
        f'aulatap311.blueprints.{name}.{name}:init_app')
...
def plug_database():
    ...
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if 'aulatap311.ext.database:init_app' in settings['default']['EXTENSIONS']:
    ...
    with open(os.path.join(os.getcwd(), 'aulatap311', 'ext', 'database.py'), 'w') as db_module:
        db_template = get_template('database.pyt')
        db_module.write(db_template.render(name='aulatap311'))
    # models.py (basic example)
    with open(os.path.join(os.getcwd(), 'aulatap311', 'models.py'), 'w') as models_file:
        mod_template = get_template('models.pyt')
        models_file.write(mod_template.render(project='aulatap311'))
    # settings.toml
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    settings['default']['EXTENSIONS'].append(
        'aulatap311.ext.database:init_app')
    ...
    cmd = ''
    if os.name == 'posix':
        cmd = f'. .venv/bin/activate; export FLASK_APP=aulatap311.app; export FLASK_ENV=development; flask db init'
    elif os.name == 'nt':
        cmd = f'call .venv/Scripts/activate; set FLASK_APP=aulatap311.app; set FLASK_ENV=development; flask db init'
    subprocess.run(cmd, shell=True)
...
def db_migrate(message):
    settings = toml.load(os.path.join(
        os.getcwd(), 'instance', 'settings.toml'))
    if 'aulatap311.ext.database:init_app' not in settings['default']['EXTENSIONS']:
